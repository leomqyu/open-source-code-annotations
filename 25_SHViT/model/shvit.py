...
class SHSA(torch.nn.Module):
    ...
    def __init__(self, dim, qk_dim, pdim):
        ...
        self.qk_dim = qk_dim    # dim of q, k
        self.dim = dim
        self.pdim = pdim    # number of channels used in attention, also the dim of v
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        x1, x2 = torch.split(x, [self.pdim, self.dim - self.pdim], dim = 1) # x2 is untouched, as if x1 is the whole input
        x1 = self.pre_norm(x1)
        qkv = self.qkv(x1)

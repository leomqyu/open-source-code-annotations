...
class MHLA4DiT(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.block_size = kwargs["block_size"] if "block_size" in kwargs else 49
        self.block_len = int(self.block_size ** 0.5)
    ...
    def _mlp_lepe(self, x):
        ...
        lepe = self.lepe(rearrange(v, 'b (h w) (p1 p2) d -> b d (h p1) (w p2)', h=self.pieces_len, w=self.pieces_len, p1=self.block_len, p2=self.block_len))
        lepe = rearrange(lepe, 'b d (h p1) (w p2) -> b (h w) (p1 p2) d', h=self.pieces_len, w=self.pieces_len, p1=self.block_len, p2=self.block_len)
        return q, k, v,lepe
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        normalizer = self.piece_attn(torch.matmul(q, k_sum)) + self.eps # [B*H, num_pieces, block_size, 1]

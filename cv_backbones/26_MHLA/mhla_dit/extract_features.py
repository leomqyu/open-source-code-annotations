...
class CustomImageFolderDataset(torch.utils.data.Dataset):
    """
    Wraps torchvision.datasets.ImageFolder, allowing custom logic in __getitem__.
    """
    def __init__(self, root, transform=None):
        self.image_folder = ImageFolder(root, transform=transform)
        # custom_fn: function(img, label, idx) -> (img, label)
        self.trans_no_flip = transforms.Compose([
        ...
        self.trans_flip = transforms.Compose([
            transforms.Lambda(lambda pil_image: center_crop_arr(pil_image, args.image_size)),
    ...
    def __getitem__(self, idx):
        ...
        return self.trans_no_flip(img), self.trans_flip(img), label
################################################################################
...
def main(args):
    ...
    #     transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5], inplace=True)
    # ])
    dataset = CustomImageFolderDataset(args.data_path)
    ...
    train_steps = 0
    for x_no_flip, x_flip, y in tqdm(loader):
        x_no_flip = x_no_flip.to(device)
        x_flip = x_flip.to(device)
        y = y.to(device)
        with torch.no_grad():
            # Map input images to latent space + normalize latents:
            x_no_flip = vae.encode(x_no_flip).latent_dist.sample().mul_(0.18215)
            x_flip = vae.encode(x_flip).latent_dist.sample().mul_(0.18215)
        x = torch.cat([x_no_flip, x_flip])

...
from fla.ops.mhla.naive import naive_chunk_simple_mhla_fixed, naive_recurrent_mhla
...
class MHLA(nn.Module):
    ...
    def __init__(
        ...
        L = 32
        lower_tri = torch.tril(torch.ones(L, L, dtype=torch.float32))
        row_counts = torch.arange(L, dtype=torch.float32).unsqueeze(1) + 1.0
        lower_tri = lower_tri / row_counts
        self.mixing_matrix = nn.Parameter(lower_tri.view(L, L, 1, 1, 1, 1))
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        # clamp the mixing matrix weight into [0, 1]
        self.mixing_matrix.data = torch.clamp(self.mixing_matrix.data, 1e-5, 1).tril()
        ...
        if self.feature_map_k is not None and self.feature_map_q is not None:
        ...
        if recurrent_state is not None:
            print('Recurrent state is not None! The shape is: ', recurrent_state.shape)
        if mode == 'fused_recurrent':
            o, recurrent_state = naive_recurrent_mhla(
                q=q,
                k=k,
                v=v,
            ...
            o = naive_chunk_simple_mhla_fixed(
                q=q,
                k=k,
                v=v,
                mixing_matrix=self.mixing_matrix,

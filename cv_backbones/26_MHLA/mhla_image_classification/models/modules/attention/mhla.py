...
class MHLA_Normed_Torch(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.norm(x)    # layernorm, (B, N: num of pieces, W: window size / patches per piece, C)
        B, N, W, C = x.shape
        H = self.num_heads
        D = self.head_dim
        ...
        q, k, v = self._process_qkv_impl(q, k, v, B, N, H, D)   # (B, num_head, N, W, head_dim)

        kv = torch.matmul(k, v)  # [B*H, num_pieces, head_dim, head_dim]
        kv = self.piece_attn(kv)  # [B*H, num_pieces, head_dim, head_dim]

...
def save_checkpoint(
    ...
    elif accelerator is not None and accelerator.distributed_type == DistributedType.DEEPSPEED:
        return save_checkpoint_deepspeed(
            work_dir=work_dir,
            epoch=epoch,
            model=model,
            accelerator=accelerator,
...
def save_checkpoint_deepspeed(
    ...
    if accelerator.is_main_process:
        ...
        torch.save({"state_dict": model.module.state_dict()}, model_link_path)
...
def load_checkpoint(
    ...
    if FSDP:
        ...
        return load_checkpoint_deepspeed(
            checkpoint=checkpoint,
            model=model,
            remove_state_dict_keys=remove_state_dict_keys,
        )
...
def load_checkpoint_deepspeed(
    ...
    if ".pth" in checkpoint:
        ...
        state_dict_model = find_model(f"{checkpoint}.pth")
        state_dict_model = state_dict_model.get("state_dict", state_dict_model)

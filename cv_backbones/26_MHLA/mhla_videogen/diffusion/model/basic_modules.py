...
class GLUMBConv2SeqTemp(GLUMBConv):
    def __init__(
        self,
        in_features: int,
        hidden_features: int,
    ...
    def forward(self, x: torch.Tensor, HW=None, **kwargs) -> torch.Tensor:
        ...
        x_reshaped = x.view(B, T, C, H * W).permute(0, 2, 1, 3)
        x_out = x_reshaped + self.t_conv(x_reshaped)
        x_out = x_out + self.t_conv2(x_out)
...
class GLUMBConvTemp2(GLUMBConv):
    def __init__(
        self,
        in_features: int,
        hidden_features: int,
        ...
        nn.init.zeros_(self.t_conv.weight)
        # nn.init.zeros_(self.depth_t_conv.weight)
        # nn.init.zeros_(self.depth_t_conv.bias)
    ...
    def forward(self, x: torch.Tensor, HW=None) -> torch.Tensor:
        ...
        x = self.inverted_conv(x)  # BT, C, H, W

        # temporal aggregation
        x_reshaped = x.view(B, T, -1, H * W).permute(0, 2, 1, 3)  # B, C, T, H*W
        x_temp = self.depth_t_conv(x_reshaped).permute(0, 2, 1, 3).view(B * T, -1, H, W)
...
class GLUMBConv3D(GLUMBConv):
    ...
    def _load_state_dict_pre_hook(
        ...
        # for depth_conv.conv.weight
        depth_conv_key = prefix + "depth_conv.conv.weight"
        if depth_conv_key in state_dict:
            current_shape = self.depth_conv.conv.weight.shape
            checkpoint_shape = state_dict[depth_conv_key].shape

            if len(checkpoint_shape) == 4 and len(current_shape) == 5:
                print(f"Converting {depth_conv_key}: {checkpoint_shape} -> {current_shape}")

                # convert 2D to 3D
                out_c, in_c, h, w = checkpoint_shape
                new_weight = torch.zeros(
                    current_shape, dtype=state_dict[depth_conv_key].dtype, device=state_dict[depth_conv_key].device
                )

                # put 2D weight in the center of time dimension
                center_t = current_shape[2] // 2
                new_weight[:, :, center_t, :, :] = state_dict[depth_conv_key]

                # replace the weight in state_dict
                state_dict[depth_conv_key] = new_weight
    ...
    def forward(self, x: torch.Tensor, HW=None) -> torch.Tensor:
        ...
        x = self.inverted_conv(x)  # B*T, C, H, W
        x = x.reshape(B, T, -1, H, W).permute(0, 2, 1, 3, 4)  # B, C, T, H, W

        x = self.depth_conv(x)  # B, C, T, H, W
        ...
        x = x.permute(0, 2, 1, 3, 4).reshape(B * T, -1, H, W)
        x = self.point_conv(x)

...
from diffusion.model.basic_modules import GLUMBConv, GLUMBConv3D, GLUMBConvTemp, Mlp
...
from .mhla_utils import BlockDistanceConv3D, MHLA_Video_Uni
from einops import rearrange
# import ipdb
# from fla.modules.activations import SwishFunction
...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs, block_mask=None):
        ...
        # print(f" After Flash Attn, x dtype {x.dtype}")
        # output
...
class WanLinearAttention(WanSelfAttention):
    ...
    def __init__(self, dim, num_heads, window_size=(-1, -1), qk_norm=True, eps=1e-6, **kwargs):
        ...
        print("[INFO] without_rope: ", self.without_rope)
    ...
    def attn_matmul(self, q, k, v: torch.Tensor) -> torch.Tensor:
        ...
        #     self.qkv_store_buffer["q"] = q.permute(0, 3, 1, 2)[1].cpu()  # b, n, h, h_d
        #     self.qkv_store_buffer["k"] = k.permute(0, 2, 1, 3)[1].cpu()  # b, n, h, h_d
        #     self.qkv_store_buffer["v"] = v.permute(0, 3, 1, 2)[1].cpu()  # b, n, h, h_d

        v = F.pad(v.float(), (0, 0, 0, 1), mode="constant", value=self.PAD_VAL)
        vk = torch.matmul(v, k)
        out = torch.matmul(vk, q)  # b, h, h_d, n

        # print(f"denominator max {out[:, :, -1:].abs().max()}, denominator std {out[:, :, -1:].std()}")

        # if out.dtype in [torch.float16, torch.bfloat16]:
        #     out = out.float()
        norm_out = out[:, :, :-1] / (out[:, :, -1:] + self.eps)
        # print(f"out max {norm_out.abs().max()}, out std {norm_out.std()}")
        # if norm_out.abs().max() > 1000:
        # import ipdb; ipdb.set_trace()
        return norm_out
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs):
        ...
        # print("[INFO] rope after: ", rope_after)
        # import ipdb; ipdb.set_trace()
        if rope_after:
...
class Gated_MHLA_Video(nn.Module):
    def __init__(
        self,
        dim,
        num_heads=8,
        ...
        **kwargs,
        ...
        """
        super(Gated_MHLA_Video, self).__init__()
        ...
        self.g_fn = nn.SiLU()
        self.g_norm = WanRMSNorm(dim, eps=eps)
    ...
    def forward(self, x: torch.Tensor, seq_lens, grid_sizes, freqs) -> torch.Tensor:
        ...
        g = self.g(x)
        out = self.g_norm(out) * self.g_fn(g)
            
        return self.o(out)


class MHLA_Video_Nope(nn.Module):
    def __init__(
        self,
        dim,
        num_heads=8,
        ...
        **kwargs,
        ...
        Args:
            ...
            transform: Transform function to apply to the distances
        """
        super(MHLA_Video_Nope, self).__init__()
    ...
    def forward(self, x: torch.Tensor, seq_lens, grid_sizes, freqs) -> torch.Tensor:
        ...
        return self.out_rmsnorm(self.o(out))



class Gated_MHLA_Video_LePE(nn.Module):
    def __init__(
        self,
        dim,
        num_heads=8,
        ...
        **kwargs,
        ...
        Args:
            ...
            transform: Transform function to apply to the distances
        """
        super(Gated_MHLA_Video_LePE, self).__init__()
    ...
    def forward(self, x: torch.Tensor, seq_lens, grid_sizes, freqs) -> torch.Tensor:
        ...
        return self.o(out + lepe)


class MHLA_Video_LePE(nn.Module):
    def __init__(
        self,
        dim,
        num_heads=8,
        ...
        **kwargs,
        ...
        Args:
            ...
            transform: Transform function to apply to the distances
        """
        super(MHLA_Video_LePE, self).__init__()
    ...
    def forward(self, x: torch.Tensor, seq_lens, grid_sizes, freqs) -> torch.Tensor:
        ...
        return self.out_rmsnorm(self.o(out + lepe))

class MHLA_Video(nn.Module):
    def __init__(
        self,
        dim,
        num_heads=8,
        ...
        **kwargs,
        ...
        Args:
            ...
            transform: Transform function to apply to the distances
        """
        super(MHLA_Video, self).__init__()
...
WAN_SELFATTENTION_CLASSES = {
    ...
    "mhla": MHLA_Video,   
    "gated_mhla": Gated_MHLA_Video, 
    "mhla_nope": MHLA_Video_Nope,
    "mhla_lepe": MHLA_Video_LePE,
    "gated_mhla_lepe": Gated_MHLA_Video_LePE,
    "mhla_uni": MHLA_Video_Uni,
}
...
class WanAttentionBlock(nn.Module):
    def __init__(
        ...
        self.self_attn = WAN_SELFATTENTION_CLASSES[self_attn_type](
            dim, num_heads, window_size, qk_norm, eps, rope_after=rope_after, without_rope=without_rope, power=power, out_rmsnorm=rms_output, normalize_out=norm_output, is_gated=is_gated, is_lepe=is_lepe, block_layout=block_layout
        )
        ...
        elif ffn_type == "GLUMBConv3D":
            self.skip_ffn = GLUMBConv3D(
                in_features=dim,
                hidden_features=ffn_dim,
                use_bias=(True, True, False),
                norm=(None, None, None),
    ...
    def forward(
        ...
        # x = cross_attn_ffn(x.to(x_dtype), context, context_lens, e)
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def load_base_model_peft_ckpt(self, pretrained_model_path, init_patch_embedding=False, verbose=True):
        ...
        # # Assert that this is a PEFT-wrapped model
        # from peft import PeftModel
        # assert isinstance(self, PeftModel), "This function is only for PEFT-wrapped models. Use load_model_ckpt for regular models."
        # logger.info("======> Confirmed PEFT-wrapped model <=======")
    ...
    def load_model_ckpt(self, pretrained_model_path, init_patch_embedding=False, verbose=True, enable_lora=False):
        ...
        if pretrained_model_path.endswith(".safetensors"):
            ...
            pretrained_model_state_dict = torch.load(pretrained_model_path, map_location="cpu")
        if "state_dict" in pretrained_model_state_dict:
            pretrained_model_state_dict = pretrained_model_state_dict["state_dict"]
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        for i in tqdm(range(0, len(param_items), chunk_size)):
            ...
            # Clear memory
            # torch.cuda.empty_cache() if param_device.startswith('cuda') else None
...
class WanLinearAttentionModel(WanModel):
    ...
    def __init__(
        ...
        attn_type="flash",  # flash, linear, mllalinear, flex, mhla
        ffn_type="mlp",
        rope_after=False,
        without_rope=False,
        mhla_adjust=False,
        ...
        # output self_attn_type

        print(f"[INFO] WanLinearAttentionModel: self_attn_types: {self_attn_types}")

        self.self_attn_types = self_attn_types
        self.diagonal_mask = None
        self.repo_after = rope_after
        self.power = power
        self.without_rope = without_rope
        self.mhla_adjust = mhla_adjust
    ...
    def _adjust_hw_after_patch(self, t: torch.Tensor) -> torch.Tensor:
        r"""
        Ensure the last two dims (H, W) of t (shape [1, D, F, H, W])
        satisfy multiples of block_layout[1] and block_layout[2].
        Special case: if (H, W) == (15, 26), crop to (15, 25) instead of pad.
        """
        assert t.dim() == 5 and t.size(0) == 1, "expect tensor shape [1, D, F, H, W] after patch_embedding"
        _, _, _, h, w = t.shape
        # print('t:', t.shape)
        ...
        mul_h = self.block_layout[1]
        mul_w = self.block_layout[2]

        target_h = ((h + mul_h - 1) // mul_h) * mul_h
        target_w = ((w + mul_w - 1) // mul_w) * mul_w
        ...
        # pad order for 5D: (w_left, w_right, h_top, h_bottom)
    ...
    def forward(self, x, timestep, context, seq_len, clip_fea=None, y=None, block_mask=None, **kwargs):
        ...
        # import ipdb; ipdb.set_trace()
        # print(f"save_block_output: {self.save_block_output}")
        # if self.save_block_output:
        #     self.block_output_buffer[self.inference_timestep] = {}
        #     self.register_block_hook(device="cpu", score_only=True)
        ...
        if self.mhla_adjust:
            # if x.shape[-1] != 50:
            #     x = [self._adjust_hw_after_patch(u) for u in x]
            seq_len = 31500
        # import ipdb; ipdb.set_trace()
        ...
        if block_mask is None and self.diagonal_mask is not None:
            # flex attention, use self.diagonal_mask as block mask
            # bsa: block sparse attention, use input block_mask
            seq_len = x.shape[1]
...
def init_model_configs(model_cfg, vae_cfg):
    ...
    basic_config["without_rope"] = model_cfg.without_rope
    basic_config["power"] = model_cfg.power
    basic_config["ffn_type"] = model_cfg.ffn_type
    basic_config["mhla_adjust"] = model_cfg.mhla_adjust
    basic_config["rms_output"] = model_cfg.rms_output
    basic_config["norm_output"] = model_cfg.norm_output

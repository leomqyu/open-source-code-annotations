...
class UniPC:
    def __init__(
        self, model_fn, condition, uncondition, cfg_scale, flow_shift=3.0, model_kwargs=None, condition_as_list=False
    ):
        self.model = model_fn
        ...
        self.condition_as_list = condition_as_list
    ...
    def sample(self, latents, steps=28):
        ...
        if do_classifier_free_guidance:
            if self.condition_as_list:
                # for Wan, the context is list
                if isinstance(self.uncondition, list):
                    # NOTE support bs > 1 for Wan
                    assert isinstance(self.condition, list) and len(self.uncondition) == len(
                        self.condition
                    ), "unconditional_condition and condition must have the same length"
                    prompt_embeds = [self.uncondition[i].to(latents.dtype) for i in range(len(self.uncondition))] + [
                        self.condition[i].to(latents.dtype) for i in range(len(self.condition))
                    ]
                else:
                    prompt_embeds = [self.uncondition.to(latents.dtype), self.condition.to(latents.dtype)]
            else:

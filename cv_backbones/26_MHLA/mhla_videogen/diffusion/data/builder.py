...
def build_dataloader(dataset, batch_size=256, num_workers=4, shuffle=True, dataloader_type="video", **kwargs):
    ...
    #         if kwargs.get('batch_sampler'):
    #             sampler = kwargs.get('batch_sampler').sampler
    #         else:
    #             sampler = kwargs.get('sampler')

    #         if dataloader_type == "video":
    #             env_key = f'CURRENT_VIDEO_STEP_START_RANK_{rank}'
    #         else:
    #             env_key = f'CURRENT_IMAGE_STEP_START_RANK_{rank}'

    #         if hasattr(sampler, 'step_start') and hasattr(sampler, 'set_start'):
    #             step_start_from_env = int(os.environ.get(env_key, '0'))
    #             print(f"Rank {rank}, Worker {worker_id} ({dataloader_type}): setting step_start to {step_start_from_env}")
    #             sampler.set_start(step_start_from_env)

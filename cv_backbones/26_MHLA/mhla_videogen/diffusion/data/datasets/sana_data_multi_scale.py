...
class SanaWebDatasetMS(SanaWebDataset):
    ...
    def getdata(self, idx):
        ...
        if "unsplash" in data["__shard__"]:
            # used to mitigate the domain gap between different datasets
            txt_fea = f"Experience the striking realism and cinematic texture of this image. Authentically styled cinematic photo capturing {txt_fea}. Showcases 35mm photography, vibrant bokeh, professional quality, 4k resolution, and extraordinarily detailed imagery."
...
@DATASETS.register_module()
class SanaWebDatasetMSControl(SanaWebDatasetMS):
    def __init__(self, *args, **kwargs):
        self.control_signal_type = kwargs.get("control_signal_type", None)
        assert self.control_signal_type in [
            "scribble"
        ], f"control_signal_type must be one of ['scribble'] but got {self.control_signal_type}"

        self.data_dir = kwargs.get("data_dir", "")
        assert len(self.data_dir) == 1, "controlnet data dir must be one"
    ...
    def getdata(self, idx):
        ...
        self.transform_mask = self.transform

        # add control signal
        control_signal = self.get_control_signal(dataindex_info)
    ...
    def get_control_signal(self, dataindex_info):
        """
        data:
        {
            '.json': {
                'height': 1024,
                'width': 1024,
                'prompt': 'In Pixar 3D-style image of a 8 year old, very pretty African American girl',
                'url': 'https://0_2.png',
                'prompt_prob': 0.25377
                },
            '.png': <PIL.PngImagePlugin.PngImageFile image mode=RGB size=1024x1024>,
            '__key__': 0_2',
            '__dataset__': None,
            '__index__': 0,
            '__shard__': '/path/to/controlnet_data/00000000.tar',
            '__shardindex__': 0
        }
        """

        data_dir = self.data_dir[0]
        tar_name = dataindex_info["shard"].split("/")[-1].split(".")[0]
        base_path = os.path.join(data_dir, self.control_signal_type, tar_name)
        search_pattern = os.path.join(base_path, f"{dataindex_info['key']}_thickness_*.png")
        matching_files = glob.glob(search_pattern)
        if not matching_files:
            raise FileNotFoundError(f"No control signal file found matching pattern: {search_pattern}")

        control_signal_path = matching_files[0]
        assert len(matching_files) == 1, f"There should be only one control signal file, but got {len(matching_files)}"

        control_signal = Image.open(control_signal_path)
        if self.transform_mask:
            control_signal = self.transform_mask(control_signal)

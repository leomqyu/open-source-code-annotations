...
class FcaBlock(nn.Module):
    ...
    def forward(self, x, s=None):
        ...
        # MYNOTE: the "cross multihead attn" part: just for kv, attach the s
        if self.inner_depth > 0:    # inner depth is the depth of block inside that stage. if 0 means the first block of the stage, then no global token
            s = s * self.s_scale_factors
            _, N_S, _ = s.shape
            x, s = self.norm1(x), self.s_norm(s)
            qkv_x = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)    # (3, B, num_head, N, head_dim)
            qkv_s = self.qkv(s).reshape(B, N_S, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            q_x, k_x, v_x = qkv_x[0], qkv_x[1], qkv_x[2]    # (B, num_head, N, head_dim)
            k_s, v_s = qkv_s[1], qkv_s[2]                   # glob toks, (B, num_head, N, head_dim)
        ...
        if self.s_up:
            s_update = self.dw_conv(x)  # MYNOTE: the new s is from stride4 attn of the new x, not the original one
            s_update = s_update.view(B, C, -1).transpose(1,2)
        x = self.local_conv(x)  # MYNOTE: 3x3dw
        x = x.view(B, C, N).transpose(1,2)

        # channel mixer
        # MYNOTE: the FFN part, mlp is 2 layer mlp w. activationhe
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            if i==0:
                ...
                # MYNOTE: concat all the s (glob) until max, along tok
...
class FcaFormer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # shallow layers
        # MYNOTE: still use conv at shallower layers
        x = self.layer_0(x)
        x = self.layer_1(x)

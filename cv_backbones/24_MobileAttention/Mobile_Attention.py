...
class Mobile_Attention(nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        B, L, _ = queries.shape
        _, S, _ = keys.shape
        queries = self.query_projection(queries).view(B, L, self.n_heads, -1)   # [B, L, self.n_heads, d_model // n_heads]
        keys = self.key_projection(keys).view(B, S, self.n_heads, -1)           # [B, L, self.n_heads, d_model // n_heads]
        values = self.value_projection(values).view(B, S, self.n_heads, -1)     # [B, S, self.n_heads, d_model // n_heads]
        queries = queries.transpose(1, 2)   # [B, self.n_heads, L, d_model // n_heads]
        keys = keys.transpose(1, 2)         # [B, self.n_heads, L, d_model // n_heads]
        values = values.transpose(1, 2)     # [B, self.n_heads, S, d_model // n_heads]

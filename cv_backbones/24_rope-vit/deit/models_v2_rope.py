...
# MYNOTE: fn to apply rope to q and k
def apply_rotary_emb(
    xq: torch.Tensor,   # MYNOTE: [B, num_heads, N, head_dim]
    xk: torch.Tensor,   #         [B, num_heads, N, head_dim]
    freqs_cis: torch.Tensor,    # (N, hd/2) or (nh, N, hd/2), depending on whether are mixed rope (head specific, this implementation)
) -> Tuple[torch.Tensor, torch.Tensor]:
    xq_ = torch.view_as_complex(xq.float().reshape(*xq.shape[:-1], -1, 2))  # [B, num_heads, N, head_dim/2, 2]
    xk_ = torch.view_as_complex(xk.float().reshape(*xk.shape[:-1], -1, 2))  # [B, num_heads, N, head_dim/2, 2]
    freqs_cis = reshape_for_broadcast(freqs_cis, xq_)   # (1, 1, N, hd/2) if shared or (1, nh, N, hd/2) (if using Mixed RoPE-- head specific)
    xq_out = torch.view_as_real(xq_ * freqs_cis).flatten(3)
    xk_out = torch.view_as_real(xk_ * freqs_cis).flatten(3) # after view_as_real: (B, nh, N, hd/2, 2) -> (B, nh, N, head_dim)
...
class RoPEAttention(Attention):
    ...
    def forward(self, x, freqs_cis):
        ...
        # MYNOTE: 1. here to apply the rope to q and k with apply_rotary_emb()
        # first dim is of dim=2 is CLS token, no rope 
...
class rope_vit_models(vit_models):
    def __init__(self, rope_theta=100.0, rope_mixed=False, use_ape=False,
        ...
        # MYNOTE: the corrent case, mixed rope
        if self.rope_mixed:
    ...
    def forward_features(self, x):
        ...
        if self.rope_mixed:
            # MYNOTE: calculate freqs_cis for each run, separatedly for each block

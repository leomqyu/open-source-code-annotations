...
class dense_vit(VisionTransformer):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self.encoder(x)
        # MYNOTE: does all the embedding, add CLS, add pe, etc
        # shape: [B, 1 + N, 768]
        
        if self.cached_kernel is None:
            self.cached_kernel = self.gaussian_kernel_1d(768, 768 ** 0.5).to(x.device).unsqueeze(0).unsqueeze(0)
        x_detach = x[:, 1:]     # MYNOTE: patch token only, [B, N, 768]
        x = torch.fft.fft(x[:, 1:], dim=-1) # MYNOTE: convert to freq, at the last dim: each tok emb as a 1d signal
        gs_k = self.cached_kernel.to(x.device)
        x = torch.fft.fftshift(x, dim=-1)
        x = x * (gs_k)
        x = torch.fft.ifftshift(x, dim=-1)  # MYNOTE: apply a low-pass Gaussian filter in the frequency domain, only low freq components maintained
        x = torch.fft.ifft(x, dim=-1).real
        diff =  x_detach / torch.abs(x - x_detach)  # MYNOTE: if smoothing don't change tok -> diff is big -> should be chosen as top-k: are the main body tok 

...
class FocusedLinearAttention(nn.Module):
    def __init__(self, dim, num_patches, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,
        ...
        self.focusing_factor = focusing_factor  # the p in the paper
        ...
        self.positional_encoding = nn.Parameter(torch.zeros(size=(1, num_patches // (sr_ratio * sr_ratio), dim)))   
            # first `1`: all batch same PE, second dim is the length of k: pos enc only added to k
            # num_patches is just n
        print('Linear Attention sr_ratio{} f{} kernel{}'.
              format(sr_ratio, focusing_factor, kernel_size))

    def forward(self, x, H, W):
        B, N, C = x.shape   # Batch_size, N = H * W, channel dim
        q = self.q(x)   # [B, N, C]

        if self.sr_ratio > 1:       # if do spatial reduction, q is still same N; k, v have different N
            x_ = x.permute(0, 2, 1).reshape(B, C, H, W)
            x_ = self.sr(x_).reshape(B, C, -1).permute(0, 2, 1)
            x_ = self.norm(x_)
            kv = self.kv(x_).reshape(B, -1, 2, C).permute(2, 0, 1, 3)
        else:
            kv = self.kv(x).reshape(B, -1, 2, C).permute(2, 0, 1, 3)    # equal to doing linear proj of both K and V
        k, v = kv[0], kv[1]     # [B, N, C]
        n = k.shape[1]
        ...
        v = v.reshape(B, n, self.num_heads, -1).permute(0, 2, 1, 3)     #  [B，num_heads, n, C // B，num_heads]

        print('here1', v.shape) # ([4, 8, 100, 64])

        z = 1 / (q @ k.mean(dim=-2, keepdim=True).transpose(-2, -1) + 1e-6)
        kv = (k.transpose(-2, -1) * (n ** -0.5)) @ (v * (n ** -0.5))
        x = q @ kv * z  # [B, n, C]

        print('here2', x.shape) # [4, 100, 512])
        ...
        x = self.proj(x)    # a linear layer that doesn't change shape
...
if __name__ == '__main__':
    fla = FocusedLinearAttention(dim = 512, num_patches=100)

    x = torch.randn((4, 100, 512))   # B, N, C
    h = w = 10

    res = fla(x, h, w)

    print('here4', res.shape)   # [4, 100, 512]

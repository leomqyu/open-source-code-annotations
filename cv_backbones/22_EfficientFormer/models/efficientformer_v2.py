...
# MYNOTE: the attention module used in later blocks
# input: (B, dim, H, W) -> output same shape
# if there is stride, it just mean that do attn at low resolution then upsample back, 
#     doesn't mean that the input and output are diff shape
# In this Eformer implementation, key dim is set to fixed of 32 (per head, 8 heads in total) no matter which stage
#     The dim of q,k is num_head*key_dim = 8*32=256
#     The dim of v is num_head*key_dim*attention_ratio=8*32*4=1024
#     Then for the final ouput, project the dimension back
# The norm is implemented in the token mixer, after every channel mixing
class Attention4D(torch.nn.Module):
    def __init__(self, 
                 dim=384, 
                 key_dim=32,    # Dimension of the key and query vectors per head
                 num_heads=8,
                 attn_ratio=4,  # Expansion ratio for the value dimension
                 resolution=7,  # Spatial resolution of the input feature map (e.g., 7 for a 7x7 feature map)
                 act_layer=nn.ReLU, 
                 stride=None
                ):
        super().__init__()
        ...
        # MYNOTE: a conv, added after the whole (QK^T)V attention
        self.v_local = nn.Sequential(nn.Conv2d(self.num_heads * self.d, self.num_heads * self.d,
                                               kernel_size=3, stride=1, padding=1, groups=self.num_heads * self.d),
                                     nn.BatchNorm2d(self.num_heads * self.d), )

        # MYNOTE: after attn, the shape is (B, num_heads, N, N), 
        # so talking head is like a channel mixing where head is channel
    ...
    @torch.no_grad()
    def train(self, mode=True): # override the train method
        super().train(mode)
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        # (B, self.num_heads, N, self.key_dim)
        q = self.q(x).flatten(2).reshape(B, self.num_heads, -1, self.N).permute(0, 1, 3, 2)
        # actually K^T: (B, self.num_heads, self.key_dim, N)
...
# MYNOTE: 
# H, W shrink 4 times; channel in_chs -> out_chs
...
# MYNOTE:
# To generate the Q in the subsample module's Q
# input shape (B, in_dim, H, W) -> output (B, out_dim, H//2, W//2)
# for downsample with H, W shrinking by 2, 
class LGQuery(torch.nn.Module):
    def __init__(self, in_dim, out_dim, resolution1, resolution2):
        ...
        self.pool = nn.AvgPool2d(1, 2, 0)   # kernel_size, stride, padding
...
# MYNOTE:
# downsample using the attention method
# input: (B, dim, H, W) -> output (B, dim, H//2, W//2)
...
# MYNOTE:
# The downsample module between different blocks
# input: (B, dim, H, W) -> output (B, dim, H//2, W//2)
class Embedding(nn.Module):
    def __init__(self, patch_size=3, stride=2, padding=1,
        ...
        super().__init__()
        self.light = light  # normal downsample using d3x3 then 1x1
        self.asub = asub    # to use the Attention4DDownsample based downsample
                            # else, to use dense conv
...
# if mid_conv=False, transformer like: 1x1 -> 1x1
# if mid_conv=True,  1x1 -> d3x3 -> 1x1
class Mlp(nn.Module):
    """
...
# The attn + ffn block used as blocks for later stages
# input: [B, C, H, W] -> output [B, C, H, W]
...
# The ffn-only block used as blocks for early stages
# input: [B, C, H, W] -> output [B, C, H, W]
class FFN(nn.Module):
...
def eformer_block(# MYNOTE: 
                  # index is the the index of the stage (in total 4 stages, 0,1,2,3);
                  # layers is the config param, where layers[index] is the number of blocks in that stage
                  dim, index, layers,   
    ...
    for block_idx in range(layers[index]):
        ...
        # MYNOTE
        # attention incorporated FFN: 
        # if (stage 3 and 4) && (block_idx+1 > num_blocks_in_this_stage - vit_num)
        # i.e., for stage 3 and 4, only the last vit_num blocks are attn based
        if index >= 2 and block_idx > layers[index] - 1 - vit_num:
            if index == 2:
                stride = 2  # if stage 3, do half resolution attention then upsample back
            else:
                stride = None
...
class EfficientFormerV2(nn.Module):
    def __init__(self, layers, embed_dims=None,
                 ...
                 drop_rate=0., drop_path_rate=0.,
                 use_layer_scale=True, layer_scale_init_value=1e-5,
                 fork_feat=False,   # if set false, only output single classification features; else use hierachical like for segmentation
                 init_cfg=None,
                 pretrained=None,
                 vit_num=0,
                 distillation=True,
        ...
        for i in range(len(layers)):
            ...
            if downsamples[i] or embed_dims[i] != embed_dims[i + 1]:
                ...
                if i >= 2:  # for stage 3 and 4, use attention based downsample
                    asub = True
                else:
                    asub = False
                network.append(

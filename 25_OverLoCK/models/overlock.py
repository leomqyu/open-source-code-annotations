...
# initial spatial map generation. reduce image length by 4 times
# e.g. in: (batch_size, 3, 224, 224); out: (batch_size, 96, 56, 56).
def stem(in_chans=3, embed_dim=96):
    return nn.Sequential(
        nn.Conv2d(in_chans, embed_dim//2, kernel_size=3, stride=2, padding=1, bias=False),  # shrink length by half
        nn.BatchNorm2d(embed_dim//2),
        nn.GELU(),
        nn.Conv2d(embed_dim//2, embed_dim//2, kernel_size=3, padding=1, bias=False),
        nn.BatchNorm2d(embed_dim//2),
        nn.GELU(),
        nn.Conv2d(embed_dim//2, embed_dim, kernel_size=3, stride=2, padding=1, bias=False), # shrink length by half
...
# scale each channel by a factor of [0, 1]
class SEModule(nn.Module):
    def __init__(self, dim, red=8, inner_act=nn.GELU, out_act=nn.Sigmoid):
        """
        dim is the input channel dimension
        red is reduction ration: how many times reducing the channel dimention
        """
        super().__init__()
        inner_dim = max(16, dim // red)     # the number of channels in inner SE. 
        self.proj = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),        # reduce feat map to H=W=1
            nn.Conv2d(dim, inner_dim, kernel_size=1),   # reduce feat map's channel dim to inner_dim
            inner_act(),
            nn.Conv2d(inner_dim, dim, kernel_size=1),   # increase feat map's channel dim back to orig dim
            out_act(),  # make the scaling factor [0,1]
        )       # output a scaling factor with H=W=1, channel dim no change, 
        
    def forward(self, x):
        x = x * self.proj(x)    # in effect, scale each channel by a factor of [0, 1]
        return x


# a technique that scales each channel of the output feat map by a learnable factor 
# each channel scale by a same factor. 
# implementated as a 1x1 dwconv 
class LayerScale(nn.Module):
    def __init__(self, dim, init_value=1e-5):
        super().__init__()
        self.weight = nn.Parameter(torch.ones(dim, 1, 1, 1)*init_value, # shape of kernel weight: [in_dim, out_dim, H, W]
                                   requires_grad=True)
...
class LayerNorm2d(nn.LayerNorm):
    ...
    def forward(self, x):
        x = rearrange(x, 'b c h w -> b h w c')  # normalize only along the c dimension
        x = super().forward(x)
...
class DynamicConvBlock(nn.Module):
    def __init__(self,
        ...
        ctx_dim = ctx_dim // 4      # ctx_dim is embed_dim[-1], which is the dim of block4 output. But after reduction etc, the input context's dim is ctx_dim // 4
        out_dim = dim + ctx_dim
        mlp_dim = int(dim*mlp_ratio)
        ...
        if not is_first:
            self.x_scale = LayerScale(ctx_dim, init_value=1)    # a conv module
    ...
    def get_rpb(self):  # rpb is just that special spatial embedding
    ...
    def _forward_inner(self, x, h_x, h_r):
        """
        x is the original "z", h_x is the context "p_i", h_r is the original context p_0
        """
        input_resoltion = x.shape[2:]       # H, W
        B, C, H, W = x.shape
        B, C_h, H_h, W_h = h_x.shape    # p_i, p_0 and x already same size
        
        if not self.is_first:
            h_x = self.x_scale(h_x) + self.h_scale(h_r) # if is not first, directly ADD the current and original context

        x_f = torch.cat([x, h_x], dim=1)        # then concatenate context with feat map
        x_f = self.dwconv1(x_f)                 # 3x3 dwconv the concatenated
        identity = x_f
        x_f = self.norm1(x_f)
        x = self.fusion(x_f)                    # 3x3 conv, 1x1 conv
        ...
        # attn is the globally guided kernel weights, have a large and a small kernel
        ...
        if self.is_last:    # if it is last layer, just return the feat map (which is ), if is not, fuse p and z together for return
            return (x, None)
        else:
            l_x, h_x = torch.split(x, split_size_or_sections=[C, C_h], dim=1)
            return (l_x, h_x)   # l_x is the returned z_i, h_x is the p_i
...
class OverLoCK(nn.Module):
    ...
    '''
    def __init__(self, 
                 depth=[2, 2, 2, 2],    # how many child blocks in each (first 4) block
                 sub_depth=[4, 2],      # how many child blocks in each (last 2) block
                 in_chans=3, 
                 embed_dim=[96, 192, 384, 768],     # the first 4 blocks' chennel dim
                 kernel_size=[7, 7, 7, 7],  # kernel size in each (first 4) block
                 mlp_ratio=[4, 4, 4, 4],    # the "reversed bottleneck ratio"
                 sub_mlp_ratio=[4, 4],      # ~ for two sub blocks
                 sub_num_heads=[4, 8],
                 ls_init_value=[None, None, 1, 1],  # init param of LayerScale
                 res_scale=True,
                 smk_size=5,
                 deploy=False,
                 use_gemm=True,
        ...
        """
        block 1-3 is Base-Net, output is called `x`
        ...
        self.high_level_proj = nn.Conv2d(embed_dim[-1], embed_dim[-1]//4, kernel_size=1)    # to 1/4 channel dim by 1x1 conv
        ...
        for i in range(sub_depth[0]):
            self.sub_blocks3.append(    # only first considered is_first, always return return (l_x, h_x) ie (z_i, p_i)
                DynamicConvBlock(
                    dim=embed_dim[2],
                    ctx_dim=embed_dim[-1],
                    kernel_size=kernel_size[2],
        ...
        for i in range(sub_depth[1]):       # only the last is considered `is_last`: return (x, None); first isn't `is_first`
    ...
    def forward_sub_features(self, x, ctx):
        ...
        ctx_up = F.interpolate(ctx_ori, size=x.shape[2:], mode='bilinear', align_corners=False) # ctx in block3 with 1/4 c dim, larger H, W -- p_0 for subblock 3
        
        # no downsample at subblock3
        for idx, blk in enumerate(self.sub_blocks3):
            if idx == 0:
                ctx = ctx_up    # if first layer, p_i is p_0
            x, ctx = blk(x, ctx, ctx_up)    # x, p_i, p_0

        x, ctx = self.patch_embedx(x, ctx)  # downsample both x and ctx by 2 times
        for idx, blk in enumerate(self.sub_blocks4):
            x, ctx = blk(x, ctx, ctx_ori)       # at last, x contains a fusion of feat map and context, `ctx` will be none
        
        return (x, ctx_cls)     # output the feat map (fused with context) and the ctx generated in block3

    def forward_features(self, x):
        
        x = self.forward_pre_features(x)
        x, ctx = self.forward_base_features(x)      # x is output of block 3, ctx is output of block 4

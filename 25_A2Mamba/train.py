...
try:
    ...
    from mmcv.runner import load_state_dict as load_model
except:
    from mmengine.runner import load_checkpoint as load_ckpt
    from mmengine.runner import load_state_dict as load_model
...
def get_args_parser():
    ...
    parser.add_argument('--mesa', type=float, default=0,
                        help='enable mesa for training (default: 0)')
    parser.add_argument('--mesa-start-epoch', type=int, default=75, metavar='N',
                        help='epoch to start mesa training (default: 75)')
...
def main(args):
    ...
    # if args.finetune:
    #     load_ckpt(model, args.finetune, map_location='cpu')
    ...
    if args.finetune:
        ...
        # load_ckpt(model_ema.module, args.finetune)
        # state_dict = torch.load(args.finetune)['state_dict_ema']
        # load_model(model_ema.module, state_dict)
    ...
    if args.rank == 0:
        ...
        if args.mesa > 0 and args.mesa_start_epoch > 0:
            checkpoint_dir = f'{output_dir}/checkpoints_before_mesa/'
            os.makedirs(checkpoint_dir, exist_ok=True)
            saver_mesa = CheckpointSaver(model=model, optimizer=optimizer, 
                                        args=args, model_ema=model_ema,
                                        checkpoint_prefix='checkpoint-mesa',
                                        amp_scaler=loss_scaler, checkpoint_dir=checkpoint_dir, 
                                        recovery_dir=output_dir, decreasing=decreasing, 
                                        max_history=args.checkpoint_hist)
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            mesa = (epoch >= args.mesa_start_epoch) * args.mesa
            ...
            if (saver_mesa is not None) and (epoch == args.mesa_start_epoch):
                # save proper checkpoint with eval metric (mesa)
                save_metric = eval_metrics[eval_metric]
                saver_mesa.save_checkpoint(epoch, metric=save_metric)
...
def train_one_epoch(epoch, model, loader, optimizer, loss_fn, args,
                    ...
                    model_ema=None, mixup_fn=None, mesa=0, 
                    eta_meter=AverageMeter()):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            with detect_anomaly():
                ...
                if isinstance(output, (tuple, list, dict)):
                    ...
                    loss = loss_main + loss_aux * args.aux_loss_ratio
                    if mesa > 0:
                        with torch.inference_mode():
                            ema_output = model_ema.module(input).detach().clone()
                            ema_output = ema_output.softmax(dim=-1).detach()
                        ema_loss = loss_fn(output_main, ema_output)
                        loss = loss + ema_loss * mesa
                else:
                    loss_main = loss_fn(output, target)
                    if mesa > 0:
                        with torch.inference_mode():
                            ema_output = model_ema.module(input).detach().clone()
                            ema_output = ema_output.softmax(dim=-1).detach()
                        ema_loss = loss_fn(output, ema_output)
                        loss = loss_main + ema_loss * mesa
                    else:
                        loss = loss_main
        ...
        if not args.distributed:
            ...
            if mesa > 0:
                losses_ema_m.update(ema_loss.item(), input.size(0))
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.distributed:
                ...
                if mesa > 0:
                    reduced_loss_ema = reduce_tensor(ema_loss.data, args.world_size)
                    losses_ema_m.update(reduced_loss_ema.item(), input.size(0))
            ...
            if args.local_rank == 0:
                ...
                #     'Loss (Aux): {loss.val:#.4g} ({loss.avg:#.3g})  '
                #     'ETA: {eta}  '
                #     'Memory: {gpu_memory:d}  '
                ...
                ema_loss_info = (
                    'Loss (EMA): {loss_ema.val:#.4g} ({loss_ema.avg:#.3g})  '
                )
                ...
                _logger.info(
                    base_info.format(
                        ...
                        loss=losses_m,
                        loss_aux=losses_aux_m if is_ds else None,
                        loss_ema=losses_ema_m if mesa > 0 else None,
                        batch_time=batch_time_m,
                        rate=input.size(0) * args.world_size / batch_time_m.val,
                        rate_avg=input.size(0) * args.world_size / batch_time_m.avg,
                        lr=lr,
